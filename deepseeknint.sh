#!/usr/bin/env bash
# 🚀 Nintendo Ultra SIMULATOR 0.1.0A Installer
...
echo -e "${BLUE}=== Nintendo Ultra SIMULATOR 0.1.0A Installer ===${NC}"
sleep 1
...
declare -a APT_PACKAGES=(
  ...
  atari800 openmsx dosbox mednafen
  # Classic consoles
...
#── Done ───────────────────────────────────────────────────────────────────────
echo -e "${GREEN}\n✔ Nintendo Ultra SIMULATOR installation complete!${NC}"
echo -e "${YELLOW}⚠ BIOS/keys/firmware & ROMs are NOT included.${NC}"
echo -e "${YELLOW}ℹ For Wii U emulation (Cemu), visit https://cemu.info/ for manual setup.${NC}"
echo -e "${GREEN}Enjoy your retro gaming journey! 🎮${NC}"
